...
select first_name,last_name, addressfrom booking a join guest b on a.guest_id=b.id where room_no=101 and booking date='2016-12-03'
...
select guest_id,count(nights) "count(nights)",sum(nights)"sum(nights)" from booking where guest_id in(1185,1270) group by guest_id in(1270,1185) group by guest_id
